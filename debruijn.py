...
from mathhelper import Poly, degree, rem, to_list
...
def LFSR_from_poly(char_poly, state):
    ...
    LFSR = char_poly.all_coeffs(reverse=True)[:-1]
    next_state = state + [sum(map(lambda x, y: x & y, state, LFSR)) % 2]
    return next_state[1:]
...
def is_dbseq(sequence, order):
    ...
    if len(sequence) != (2**order):
        return False
    nseq = sequence + sequence[:order]
    nums = []
    for _ in range(2**order):
        x = int("".join(map(str, nseq[_:(_+order)])), 2)
        if x in nums:
            return False
        nums.append(x)
    return True
